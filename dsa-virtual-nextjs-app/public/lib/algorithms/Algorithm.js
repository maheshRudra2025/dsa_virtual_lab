function addLabelToAlgorithmBar(labelName) {
  // Enhanced DOM ready checking for Next.js compatibility
  ...
  if (!controlBar) {
    console.warn("AlgorithmSpecificControls not found, retrying...");
    setTimeout(function () {
      addLabelToAlgorithmBar(labelName);
    }, 100);
    return null;
...
function addControlToAlgorithmBar(type, name) {
  ...
  if (!ensureDOMReady()) {
    setTimeout(function () {
      addControlToAlgorithmBar(type, name);
    }, 50);
    return null;
  }
  ...
  // Add some styling to prevent layout issues
  if (type === "text") {
  ...
  if (!controlBar) {
    console.warn("AlgorithmSpecificControls table not found, retrying...");
    setTimeout(function () {
      addControlToAlgorithmBar(type, name);
    }, 100);
    return element;
